...
def shortest_path(source, target):
    ...
    while True:
        ...
        # Path not empty if empty
        if (frontier.empty()):
            raise Exception("There is no path")
        
        # Setup the node to check 
        ...
        if (node.state == target):
            ...
            while(node.parent is not None):
                movie_person_path.append((node.action, node.state))
                node = node.parent
            movie_person_path.reverse()
            return movie_person_path

        # Add the node to the explored state 
        explored_states.add(node.state)

        # Add the next actions to the frontier 
